...
export class WidgetCreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.deviceService.getAllDevices().subscribe((deviceList) => {
      ...
      const aggregations = this.aggregationOperations.map((agg, idx) => new AggregationBlock("aggregation_" + idx, { ...agg }))
      const operations = this.operations.map((op, idx) => new OperationBlock("operation_" + idx, { ...op }))
      const repeats = [new RepeatForBlock("repeatForBlock", { id: "repeatForBlock"})]

      this.customBlocks.push(...deviceBlocks, ...aggregations, ...operations, ...repeats)
  ...
  private _filter(value: string): DeviceInfoFragment[] {
    ...
    return this.devices.filter(d => d.name.toLowerCase().includes(filterValue));
  }

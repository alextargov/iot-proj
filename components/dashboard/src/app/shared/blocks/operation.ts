...
export class OperationBlock extends CustomBlock {
  ...
  public defineBlock() {
    ...
    if (this.hasInput) {
      this.block.appendValueInput("operation")
        .setCheck("String")
    ...
    if (this.hasSecondInput && this.additionalText) {
      this.block.appendValueInput("additional")
        .setCheck("String")
        .appendField(new Blockly.FieldLabelSerializable(this.additionalText, this.id));
    }
  ...
  public toJavaScriptCode(block: Blockly.Block): string | any[] {
    ...
    const textChildren = block.getChildren(true).map((child) => `'${child.getFieldValue('TEXT')}'`);

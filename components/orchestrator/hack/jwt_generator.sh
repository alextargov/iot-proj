...
function get_internal_tenant(){
    local INTERNAL_TENANT_ID=$(docker exec -i ${POSTGRES_CONTAINER} psql -qtAX -U "${DB_USER}" -h "${DB_HOST}" -p "${DB_PORT}" -d "${DB_NAME}" -c "SELECT id FROM tenants WHERE id = '3e64ebae-38b5-46a0-b1ed-9ccee153a0ae'")
